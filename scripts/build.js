...
async function runBuild() {
  ...
  console.log(`📂 Read ${categories.length} categories and ${calculators.length} calculators from database.js`);

  // 0. MINIFY ASSETS FOR PRODUCTION
  minifyAssets();

  // Load layout shell
  const rawLayoutHtml = fs.readFileSync(path.join(templatesDir, 'layout.html'), 'utf8');
  ...
  console.log('🎉 Site Compilation Complete! All static pre-rendered pages built successfully.');
}
...
function getCalculatorInputsOutputs(id, category) {
  return {
    inputs: `<div id="dynamic-inputs-container"></div>`,
    outputs: `<div id="dynamic-outputs-container"></div>`
  };
}
...
function buildHomepage(layoutHtml, categories, calculators) {
  ...
  const homepageTemplate = fs.readFileSync(path.join(templatesDir, 'homepage.html'), 'utf8');
  ...
  const homepageSchema = {
    ...
    "name": "CalculatorHub",
    "url": SITE_URL,
    "description": "Every Calculator You Need — Free, Fast & Accurate. Solve math, finance, loans, tax, and health calculations.",
  ...
  const finalHtml = layoutHtml
    .replace(/{{title}}/g, 'CalculatorHub - Every Calculator You Need — Free, Fast & Accurate')
    .replace(/{{description}}/g, 'Every calculator you need. 300+ free online calculators covering math, finance, health, science, and unit conversions.')
...
function buildCategories(layoutHtml, categories, calculators) {
  ...
  const categoryTemplate = fs.readFileSync(path.join(templatesDir, 'category.html'), 'utf8');
  ...
  categories.forEach(cat => {
    ...
    fs.writeFileSync(path.join(rootDir, 'categories', cat.id, 'index.html'), finalHtml, 'utf8');
  });
}
...
function buildCategoriesOverview(layoutHtml, categories) {
  ...
  fs.writeFileSync(path.join(rootDir, 'categories', 'index.html'), finalHtml, 'utf8');
}
...
function buildCalculators(layoutHtml, categories, calculators) {
  ...
  const calculatorTemplate = fs.readFileSync(path.join(templatesDir, 'calculator.html'), 'utf8');
  ...
  calculators.forEach(calc => {
    ...
    // Universal calculation scripts are dynamically managed inside the centralized UCE registry, removing file redundancy
  });
}
...
function buildAllCalculatorsList(layoutHtml, calculators) {
  ...
  fs.writeFileSync(path.join(rootDir, 'calculators', 'index.html'), finalHtml, 'utf8');
}
...
function buildStaticPages(layoutHtml, categories, calculators, articles) {
  console.log('Rendering static pages (about, contact, terms, privacy, etc)...');
          ...
          <div class="card">
            ...
            <form action="/api/contact" method="POST">
              <div class="form-group">
                ...
                <input type="text" id="c-name" name="name" class="input-control" required placeholder="John Doe">
              </div>
              <div class="form-group">
                <label for="c-email">Email Address</label>
                <input type="email" id="c-email" name="email" class="input-control" required placeholder="john@example.com">
              </div>
              <div class="form-group">
                <label for="c-msg">Message</label>
                <textarea id="c-msg" name="message" class="input-control" rows="5" required placeholder="How can we help you?"></textarea>
              </div>
  ...
  pages.forEach(p => {
    ...
    const finalHtml = layoutHtml
      ...
      .replace(/{{content}}/g, `<div class="card animate-fade-in" style="padding: 2.5rem; margin-bottom: 2rem;">${p.content}</div>`)
      .replace(/{{additional_scripts}}/g, '');
...
function buildBlog(layoutHtml, articles, calculators) {
  ...
  fs.writeFileSync(path.join(rootDir, 'blog', 'index.html'), finalIndexHtml, 'utf8');
  ...
  articles.forEach(art => {
    ...
    const finalArticleHtml = layoutHtml
      ...
      .replace(/{{content}}/g, `<div class="card" style="padding: 2.5rem; margin-bottom: 2rem;">${articleBody}</div>`)
      .replace(/{{additional_scripts}}/g, '');

    fs.writeFileSync(path.join(rootDir, 'blog', art.slug, 'index.html'), finalArticleHtml, 'utf8');
  });
}
